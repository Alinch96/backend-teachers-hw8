...
app.get("/api/teachers/selected", async (req, res) => {
  try {
    const teachers = await readTeachersFromFile();
    res.json(teachers.filter((teacher) => teacher.isSelected));
  } catch (error) {
    console.error("Помилка при отриманні вчителів:", error);
    res.status(500).json({ message: "Помилка сервера при отриманні вчителів" });
  }
...
app.post("/api/teachers", async (req, res) => {
  ...
  if (!name || !subject || !position || !phone || !email) {
    return res.status(400).json({
      message:
        "Ім'я, предмет, посада, телефон та електронна пошта є обов'язковими полями",
    });
  }
  ...
  try {
    ...
    await writeTeachersToFile([newTeacher, ...teachers]); // Зберігаємо оновлений список
